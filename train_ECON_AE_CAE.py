...
for m in all_models:
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        # ----------- Training -----------
        total_loss_train = 0
        for wafers, cond_data, weights in train_loader:
            loss_batch = cae.train_on_batch([wafers, cond_data], wafers, sample_weight = weights)
            total_loss_train += loss_batch

        # ----------- Validation -----------
        total_loss_val = 0
        for wafers, cond_data, weights in test_loader:
            loss_batch_val = cae.test_on_batch([wafers, cond_data], wafers, sample_weight = weights)
